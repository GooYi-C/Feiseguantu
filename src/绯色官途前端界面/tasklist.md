...
4. [Phase 3: 页面重构与功能整合](#phase-3-页面重构与功能整合) ✅ 已完成
...
### Phase 3 施工记录 (2025-12-09)

**施工状态**: ✅ 已完成

#### 任务 3.1: 仪表盘整合与待办事项功能增强 ✅

**完成内容**:

- 仪表盘整合「当前场景」，删除 Scene.vue 和 scene 路由
- 仪表盘添加「快捷导航」面板，替换原「统计卡片」
- 仪表盘添加「关系索引」面板，显示一把手、直接上级、配偶、靠山、绯色对象、竞争对手、政治宿敌等关系
- 仪表盘「时空舆情」面板每种信息类型使用不同颜色区分（金色/紫色/蓝色/青色/橙色/粉色）
- 地点显示从「时空舆情」面板移至顶部状态栏，与时间一同居中显示
- 移除面包屑导航（用户反馈单层导航无必要）

**用户新增需求 - 待办事项 To-Do List 样式** ⭐:

- 将「待办事项」面板改为可交互的 To-Do List 样式
- 勾选待办事项时，**真正从变量中删除**该条目（不发送给 LLM）
- 取消勾选时，**从缓存恢复**该条目到变量中
- 勾选/取消勾选后，**条目保持原位显示**，不改变排序顺序
- 已勾选条目显示删除线样式和降低透明度

**技术实现**:

- 创建 `composables/useTodoList.ts` 公共 composable，统一处理 to-do 逻辑
- 使用 `sortIndex` 机制保持条目原位显示
- 使用 `localStorage` 缓存已删除条目，支持恢复

#### 任务 3.2: 人物库卡片样式改造 ✅

**完成内容**:

- 人物库卡片改为竖向扑克牌样式（宽高比 3:4）
- 顶部显示头像，边框颜色根据性别区分（男蓝/女粉）
- 中部显示姓名、年龄、体系、级别、单位（每行一个）
- 右上角显示角色标签（从上到下排列）
- 底部显示「当前状态」字段（完整显示，不省略）
- 边框颜色根据角色类型变化（绯色对象/靠山/竞争对手/家属）
- 好感度指示器显示在底部

**用户新增需求** ⭐:

- 取消左上角男/女性别标识，改用默认头像样式区分性别
- 左上角改为显示「状态」字段（人物库.$角色名.状态）
- 年龄|体系|级别|单位 改为4行显示（每行一个），避免拥挤
- 底部「当前状态」完整显示，不使用省略号
...
- 将原 Relations.vue 页面功能集成到 Characters.vue
- 添加视图切换按钮（卡片视图/关系网视图）
- 关系网视图支持缩放、平移操作
- 使用 SVG 渲染节点和边，不同关系类型使用不同颜色
- 支持筛选功能（关系类型、性别）
- 删除 Relations.vue 和对应路由

**Bug 修复**:

- 修复筛选后边缺失问题：将边的渲染从 computed 改为直接在模板中基于 networkNodes 渲染
- 使用实际颜色值替代 CSS 变量，确保 SVG 兼容性

#### 任务 3.4 + 3.6: 资产暗账页面合并 ✅

**完成内容**:

- 将「个人资产」和「暗账」页面合并为新的「资产暗账」页面
- 删除 Assets.vue、Secrets.vue 和对应路由
- 创建 AssetsSecrets.vue，统一展示资产和暗账信息

**用户新增需求 - 布局调整** ⭐:

- 取消左侧资产概览独占一列，将申报资产/实际资产/灰色资产融入页面顶部
- 第一行：资产概览条 + 房产 + 座驾 + 白手套
- 第二行：把柄 + 政治地雷 + 人情债
- 两行高度一致
- 房产的「登记人」使用 CharacterName 组件，支持 hover/点击交互

**用户新增需求 - To-Do List 样式** ⭐:

- 把柄、政治地雷、人情债做成可交互的 To-Do List 样式
- 勾选时从变量删除，取消勾选时恢复
- 使用公共 composable `useGroupedTodoList` 处理分组逻辑

#### 任务 3.5: 派系图谱页面布局优化 ✅

**完成内容**:

- 重构 Faction.vue 页面布局，从垂直滚动改为水平分栏
- 左侧：我方派系卡片（金色边框高亮）
- 右侧：主要派系网格（自适应列数）
- 单页展示所有内容，无需滚动滚轮
...
- 优化 Opportunities.vue 页面布局
- 仅保留看板视图，移除列表/看板切换按钮
- 移除右侧数量统计（看板列头已有）
- 三列看板：潜在危机、当前机遇、待办事项
- 补全所有字段显示（来源渠道、时效性、所需资源、潜在代价、应对思路等）

**用户新增需求 - To-Do List 样式** ⭐:

- 潜在危机、当前机遇、待办事项做成可交互的 To-Do List 样式
- 勾选时从变量删除，取消勾选时恢复
- 条目保持原位显示

#### 公共功能：To-Do List Composable ✅

**创建文件**:

- `composables/useTodoList.ts` - 公共 To-Do List 逻辑
- `composables/index.ts` - 统一导出

**功能特性**:

- `useTodoList<T>` - 单类型 To-Do List（用于待办事项、政治地雷、人情债、机遇、危机）
- `useGroupedTodoList<T>` - 分组 To-Do List（用于把柄：被握/手握）
- 勾选后从变量真正删除，取消勾选后从缓存恢复
- 使用 `sortIndex` 机制保持条目原位显示
- localStorage 持久化缓存
...
| 文件路径                     | 操作 | 说明                                   |
| ---------------------------- | ---- | -------------------------------------- |
| `composables/useTodoList.ts` | 创建 | 公共 To-Do List 逻辑                   |
| `composables/index.ts`       | 创建 | 统一导出                               |
| `views/Dashboard.vue`        | 重构 | 整合场景、待办事项 To-Do、关系索引     |
| `views/Characters.vue`       | 重构 | 集成关系网视图、优化卡片样式           |
| `views/AssetsSecrets.vue`    | 创建 | 合并资产+暗账、To-Do List 样式         |
| `views/Opportunities.vue`    | 重构 | 看板视图优化、To-Do List 样式          |
| `views/Faction.vue`          | 重构 | 单页布局优化                           |
| `views/Relations.vue`        | 删除 | 合并到 Characters.vue                  |
| `views/Assets.vue`           | 删除 | 合并到 AssetsSecrets.vue               |
| `views/Secrets.vue`          | 删除 | 合并到 AssetsSecrets.vue               |
| `router.ts`                  | 修改 | 移除 relations/assets/secrets 路由     |
| `app.vue`                    | 修改 | 顶部状态栏添加地点显示、移除面包屑导航 |

#### 用户特别需求标注 ⚠️

以下功能/布局是用户明确要求的，后续施工不应改变：

1. **待办事项 To-Do 样式**：三个页面（仪表盘、资产暗账、机遇危机）的 to-do list 必须保持勾选删除变量、取消恢复的功能
2. **条目原位显示**：to-do list 勾选/取消勾选后，条目必须保持原位显示删除/恢复样式，不得改变排序
3. **角色卡4行布局**：年龄|体系|级别|单位必须分4行显示，不得合并
4. **状态角标**：角色卡左上角显示「状态」字段而非性别
5. **默认头像区分性别**：用头像样式区分男女，不使用性别角标
6. **面包屑导航移除**：用户明确要求移除，不得恢复
7. **派系页面单页布局**：无需滚动即可浏览全貌
8. **资产暗账布局**：两行等高，资产概览融入顶部而非独占一列
...
为个人档案的能力评估创建交互式雷达图组件，代替原本的进度条样式的能力评估panel，增加信息密度。
...
// 能力描述（来自游戏主prompt）:
// [
// 法则: 木桶效应(短板致死) | 时代变迁(2012前酒量称王, 2012// 后规矩为天).
//
// # 官场十维
// [公文笔杆]: 喉舌. 意图转译为红头/内参. 晋升快车道(90+=核// 心大秘).
// [揣摩上意]: 听风. 读懂"沉默"与"弦外音". 哪怕执行0分, 猜对// 方向也能软着陆.
// [资源整合]: 抓手. 调配财政/土地/编制平事. 政绩(GDP)唯一来// 源.
// [人脉经营]: 根系. 利益输送网. 决定消息灵通度&保护伞厚度.
// [政治敏感]: 嗅觉. 预判风暴/站队切割. 低分者死于换届/严打// (莫名暴毙).
// [执行魄力]: 獠牙. 脏活落地(拆迁/截访/镇压). 领导眼中的"好// 用"(酷吏).
// [酒桌功夫]: 诚意. 肝功能=忠诚度. 圈子入场券(断片前结盟).
// [魅力风度]: 脸面. 群众/媒体眼中的"官相". 女人看重的颜值. // 舆情控制&猎艳资本.
// [表演功底]: 面具. 即使恨之入骨也能握手言欢. 情绪管理失败=// 政治幼稚.
// [厚黑指数]: 脏器. 脸厚心黑. 作恶抗性(毫无波澜牺牲盟友/百// 姓).
//
// # 阶梯评价
...
在用户角色的个人档案页面与NPC人物角色卡页面增加年龄红线评语。(Phase3已增加NPC角色卡年龄红线评语，详见施工记录。)
