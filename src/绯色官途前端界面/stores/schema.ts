...
const 星期映射表: Record<string, string> = {
  周一: '周一',
  星期一: '周一',
  礼拜一: '周一',
  monday: '周一',
...
const 星期 = z
  ...
  .transform(v => {
    ...
    return 星期映射表[normalized] || 星期映射表[v] || '无';
  })
  .prefault('无');
...
export const GameSchema = z
  .object({
    ...
    人物库: z.record(z.string(), 人物Schema).prefault({}),
